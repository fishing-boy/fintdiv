...
void function_test(void)
{
	*((int*)0x40023894) |= 0x00010000;  //使能时钟
	...
	printf("sign div 0xffffffff/0\r\n");
	FINTDIV->DIVIDEND = 0xffffffff;
	FINTDIV->DIVISOR = 0;
	FINTDIV->CR |= FINTDIV_CR_DIVGO;
	while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
	printf("DIVIDEND = 0xffffffff and DIVISOR = 0\r\nQUO = %d,REMAIN = %d\r\n",FINTDIV->QUO,FINTDIV->REMAIN);

	printf("sign div 0/0xffffffff\r\n");
	FINTDIV->DIVIDEND = 0;
	FINTDIV->DIVISOR = 0xffffffff;
	FINTDIV->CR |= FINTDIV_CR_DIVGO;
	while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
	printf("DIVIDEND = 0 and DIVISOR = 0xffffffff\r\nQUO = %d,REMAIN = %d\r\n",FINTDIV->QUO,FINTDIV->REMAIN);
	
	printf("unsign div 0xffffffff/0\r\n");
	FINTDIV->DIVIDEND = 0xffffffff;
	FINTDIV->DIVISOR = 0;
	FINTDIV->CR |= FINTDIV_CR_DIVSIGN|FINTDIV_CR_DIVGO;
	while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
	...
	printf("unsign div 0/0xffffffff\r\n");
	FINTDIV->DIVIDEND = 0;
	FINTDIV->DIVISOR = 0xffffffff;
	FINTDIV->CR |= FINTDIV_CR_DIVSIGN|FINTDIV_CR_DIVGO;
	while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
