...
void function_test(void)
{
	*((int*)0x40023894) |= 0x00010000;  //使能时钟
    ...
    printf("sign div 0xffffffff\r\n");
	FINTDIV->DIVIDEND = 0xffffffff;
    FINTDIV->DIVISOR = 0xffffffff;
	FINTDIV->CR |= FINTDIV_CR_DIVGO;
    while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
    printf("DIVIDEND = 0xffffffff and DIVISOR = 0xffffffff\r\nQUO = %d,REMAIN = %d\r\n",FINTDIV->QUO,FINTDIV->REMAIN);

    printf("unsign div 0xffffffff\r\n");
	FINTDIV->DIVIDEND = 0xffffffff;
    FINTDIV->DIVISOR = 0xffffffff;
	FINTDIV->CR |= FINTDIV_CR_DIVSIGN|FINTDIV_CR_DIVGO;
    while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
	...
	FINTDIV->CR |= FINTDIV_CR_DIVGO;
    while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
    printf("DIVIDEND = 0 and DIVISOR = 0\r\nQUO = %d,REMAIN = %d\r\n",FINTDIV->QUO,FINTDIV->REMAIN);

    printf("unsign div 00\r\n");
	FINTDIV->DIVIDEND = 0;
    FINTDIV->DIVISOR = 0;
	FINTDIV->CR |= FINTDIV_CR_DIVSIGN|FINTDIV_CR_DIVGO;
    while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_DIVEND));
    ...
    printf("root int ff\r\n");
	FINTDIV->RADICAND = 0xffffffff;   
	FINTDIV->CR |= FINTDIV_CR_ROOTGO;
	while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_ROOTENDI));
    printf("RADICAND = 0xffffffff\r\nroot = %d\r\n",FINTDIV->R00T);

    printf("root float ff\r\n");
	FINTDIV->RADICAND = 0xffffffff;   
	FINTDIV->CR |= FINTDIV_CR_ROOTMOD;
	FINTDIV->CR |= FINTDIV_CR_ROOTGO;
	while(!FINTDIV_GetFlagStatus(FINTDIV_FLAG_ROOTENDI));
    printf("RADICAND = 0xffffffff\r\nroot = 0x%x\r\n",FINTDIV->R00T);
    printf("int = %d,float = %d\r\n",((FINTDIV->R00T)&0xff00)>>16,(FINTDIV->R00T)&0x00ff);
